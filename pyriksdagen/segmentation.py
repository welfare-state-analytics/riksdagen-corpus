...
def find_instances(protocol_id, archive, pattern_db, mp_db, classifier=None):
    ...
    instance_db = find_instances_xml(
        page_content_blocks, pattern_db, mp_db, classifier=classifier
    )
...
def segmentation_workflow(file_db, archive, pattern_db, mp_db, ml=True):
    ...
    if ml:
        ...
        classifier = dict(model=model, ft=ft, dim=ft.get_word_vector("hej").shape[0])
    ...
    for corpus_year, package_ids, _ in year_iterator(file_db):
        ...
        for protocol_id in progressbar.progressbar(package_ids):
            ...
            instance_db = find_instances(
                protocol_id, archive, protocol_patterns, year_mps, classifier=classifier
            )
